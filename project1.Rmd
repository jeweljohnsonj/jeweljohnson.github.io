...
description: |
  Plot you first graph with this tutorial using the ggplot2 R-package.
author:
  - name: Jewel Johnson 
date: "`r Sys.Date()`"
...
* Use the command `ggsave()` to save the graph locally. In the code below, plot is the ggplot element containing your graph. The plot will be saved in your working directory.

```{r, eval=FALSE}
library(ggplot2)
library(palmerpenguins)

plot <- ggplot(data = penguins, aes(x = species, y = body_mass_g,
                                    fill = species)) + 
  xlab("Species") + ylab("Body mass (g)") + 
  ggtitle("Body mass in three diferent species of penguins") + 
...
ggsave(plot, filename = "your_graph_name.png", width = 20, height = 20,
       units = "cm")
```
