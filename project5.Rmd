...
2. Applying selections and orderings as function of a variable
...
Perhaps the most amazing thing in making codes short and efficient is the pipe operator which is originally from the `magrittr` package which is made available for the `dplyr` package. The pipe operator helps you skip the intermediate steps of saving an object before you can use them in a command. It does so by 'piping' together results from the first object to the function ahead of the the pipe operator. The command `x %>% y %>% z` can be plainly read as 'take result of x and use it with function y and take that result and use it with function z'. This is gist of what the pipe operator does. Allow me to demonstrate.
...
The command `group_by()` allows us to group the data via existing variables. It allows for a 'split-apply-combine' way of getting output. First it will split the data or group the data with the levels in the variable, then apply the function of our choice and then finally combine the results to give us a tabular output. On its own the command doesn't do anything, we use it on conjunction with other commands to get results based on the grouping we specify. The command `ungroup()` is used ungroup the data.

## Summarising the data

### summarise()

The `summarise()` command allows you to get the summary statistics of a variable or a column in the dataset. The result is given as a tabular data. There are many types of summary statistics that can be obtained. Some of them are given below. To calculate average values it is necessary to drop `NA` values from the dataset. Use `drop_na()` command from the `tidyr` package. The comments denote what each summary statistic is.
...
The number of non `NA` values will be same as that of `n()` result as we have used `drop_na()`command in the beginning.
...
The `count()` command is used to count number of rows of a variable. Same function as that of `n()`
...
* `preserve` : Values include `TRUE` to preserve grouping in a grouped dataset and `FALSE` to not preserved grouping while sampling.
* `weight_by` : Gives priority to a particular variable during sampling. Example given below.
* `replace` : Values include `TRUE` if you want sampling with replacement which can  result duplicate values, `FALSE` if you want sampling without replacement.
...
Use `pull()` to extract columns as a vector, by name or index. Only first 10 results are shown for easy viewing.
...
Use `relocate()` to move columns to new position. Resutls are not shown as these are trivial results.
...
The function `c_across()` is similar to the earlier mentioned `across()` function. But instead of doing column wise function it applies function across columns in a row wise manner. Now most functions in R by default computes across columns, so in order to specify row wise computation, we have to explicitly use the function `rowwise()` in conjunction with other functions. In the example below we will sum both bill and flipper lengths of the penguins in the `penguins` dataset and create a new column called 'sum_of_lengths'
